...
class GameTestRunner {
  ...
  private async runSingleTest(config: TestConfiguration): Promise<void> {
    ...
    results.overallScore = (results.funFactor + (10 - results.frustrationLevel) + 
                          results.replayability + results.accessibility) / 4;
  ...
  private calculateAccessibility(config: TestConfiguration): number {
    ...
    if (config.configOverrides.gravityMultiplier && config.configOverrides.gravityMultiplier < 0.4) {
      score += 1;
    }

    // Slower enemies = more accessible
    if (config.configOverrides.enemySpeedMultiplier && config.configOverrides.enemySpeedMultiplier < 0.9) {
      score += 1;
    }
  ...
  private generateComprehensiveReport(): string {
    ...
    const sortedSessions = [...this.testSessions].sort((a, b) => b.results.overallScore - a.results.overallScore);
