...
export const useCelebrationManager = () => {
  ...
  const setCelebrationCallbacks = useCallback((callbacks: {
    onCelebrationStart?: (event: CelebrationEvent) => void;
    onCelebrationComplete?: (event: CelebrationEvent) => void;
  }) => {
  ...
  const celebrateAchievement = useCallback((achievement: Achievement) => {
    queueCelebration({
      type: 'achievement',
      priority: achievement.rarity === 'legendary' ? 100 : 
                achievement.rarity === 'epic' ? 80 :
                achievement.rarity === 'rare' ? 60 : 40,
      props: { achievement }
    });
  }, [queueCelebration]);

  const celebrateLevelVictory = useCallback((level: number, score: number, starsEarned: number) => {
    queueCelebration({
      type: 'victory',
  ...
  const celebrateComboStreak = useCallback((combo: number, multiplier: number) => {
  ...
  const celebrateBattlePassTierUp = useCallback((newTier: number, rewards: any[]) => {
    queueCelebration({
      type: 'battlepass',
      priority: 85,
  ...
  const celebrateMysteryReward = useCallback((reward: MysteryReward, x: number, y: number) => {
    ...
    const priority = reward.rarity === 'legendary' ? 95 :
                    reward.rarity === 'epic' ? 75 :
                    reward.rarity === 'rare' ? 55 : 35;
