...
export const useSocialStore = create<SocialStore>()(
  persist(
    (set, get) => ({
      ...
      updateFriend: (playerId: string, updates: Partial<Friend>) =>
        set(state => ({
          friends: state.friends.map(f => (f.player.id === playerId ? { ...f, ...updates } : f)),
        })),

      // Friend requests
      addSentFriendRequest: (request: FriendRequest) =>
        set(state => ({
          sentFriendRequests: [...state.sentFriendRequests, request],
        })),

      addReceivedFriendRequest: (request: FriendRequest) =>
        set(state => ({
      ...
      addRecentPlayer: (player: SocialPlayer) =>
        set(state => ({
          recentPlayers: [player, ...state.recentPlayers.filter(p => p.id !== player.id)].slice(
            0,
            20
          ), // Keep only recent 20 players
        })),
      ...
      // Leaderboards
      updateLeaderboard: (key: string, leaderboard: Leaderboard) =>
        set(state => ({
          globalLeaderboards:
      ...
      setPlayerRank: (category: string, rank: number) =>
        set(state => ({
          playerRankings: { ...state.playerRankings, [category]: rank },
        })),

      // Challenges
      addActiveChallenge: (challenge: SocialChallenge) =>
        set(state => ({
          activeChallenges: [...state.activeChallenges, challenge],
        })),

      updateChallenge: (challengeId: string, updates: Partial<SocialChallenge>) =>
        set(state => ({
      ...
      // Gifts
      addReceivedGift: (gift: Gift) =>
        set(state => ({
          receivedGifts: [...state.receivedGifts, gift],
          socialNotifications: state.socialNotifications + 1,
        })),

      addSentGift: (gift: Gift) =>
        set(state => ({
      ...
      updateGiftInventory: (itemType: string, amount: number) =>
        set(state => ({
          giftInventory: {
            ...state.giftInventory,
      ...
      updateGroup: (groupId: string, updates: Partial<SocialGroup>) =>
        set(state => ({
          joinedGroups: state.joinedGroups.map(g => (g.id === groupId ? { ...g, ...updates } : g)),
        })),
      ...
      addRecentShare: (share: ShareData) =>
        set(state => ({
      ...
      addSuccessfulReferral: (playerId: string) =>
        set(state => ({
      ...
      awardReferralBonus: (type: 'referrer' | 'referee', bonus: ReferralBonus) =>
        set(state => ({
...
export const useSocialConnectionStatus = () =>
  useSocialStore(state => state.socialConnectionStatus);
